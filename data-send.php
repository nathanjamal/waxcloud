<?php
	...
	//if there are more then 10 rows, delete the first one (doesnt count the actual first row, so actually deleting row 2)
	$currentRows = $listFeed->getEntries();

	if( count($currentRows) > 10 ){

		$currentRows[0]->delete();
	}
